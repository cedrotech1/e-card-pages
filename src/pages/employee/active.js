...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const fetchCards = async () => {
        try {
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/v1/card/active`, {
            headers: {
  ...
  const handleDeleteCard = async (e, cardId) => {
    ...
    const confirmDelete = window.confirm('Are you sure you want to disactivate this card?');
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success('Card disactivated successfully');
      } else {
        console.error('Failed to disactivated card:', data.message);
        toast.error(data.message);
      }
    } catch (error) {
      console.error('Error disactivated card:', error);
      toast.error('Error disactivated card');
    }
  };
                                  ...
                                  <div className="member col-xl-12" style={{ padding: "0.4cm" }}>
                                    ...
                                    {card.cardUser && (
                                      <>
                                        ...
                                        <button onClick={(e) => handleDeleteCard(e, card.id)} style={{ marginTop: '10px', backgroundColor: '#f38a7a', color: 'white', width: '70%', borderRadius: '5px', border: '0px', padding: '5px 10px' }}>Disactivate</button>
                                      </>
                                    )}
